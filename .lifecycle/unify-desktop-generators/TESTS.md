...
- **TEST-3** (tier: unit) [acceptance] [invariant: INV-1] [covers: ITEM-1, ITEM-2, ITEM-3, ITEM-4, ITEM-5] file: `src-app/ui/scripts/check-harness-parity.consumer.test.mjs` — asserts: no workspace re-forks any of the six generators or the shared surface lib (each fork path is absent on disk) AND every npm script that drives one resolves to a path under `sdk/packages/gallery/scripts/` — extends the existing `TEST-6h`.
- **TEST-4** (tier: unit) [acceptance] [invariant: INV-4] [covers: ITEM-4] file: `src-app/ui/scripts/check-harness-parity.consumer.test.mjs` — asserts: the surface enumeration EVERY workspace script now resolves to reports the `interaction` class — driven through the real `enumerateSurfaces`/`captureCells`/`cellUrl`/`surfaceCount` against a stub page exposing one interaction recipe, so a stale copy (which silently emits fewer cells) goes RED. Both workspaces' modules resolve to the same module instance.
...
- **TEST-6** (tier: unit) [acceptance] [invariant: INV-3] [covers: ITEM-6] file: `src-app/ui/scripts/check-harness-parity.consumer.test.mjs` — asserts: the re-fork proof is behavioural — re-creating a fork file on disk (temp fixture) makes the discovery assertion RED, and the generator proof reads a real spawned process's exit code + stdout rather than any file's text. Mutation control, not a positive-only guard.
