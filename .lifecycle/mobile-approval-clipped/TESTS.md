...
- **TEST-8** (tier: e2e) [acceptance] [invariant: INV-4] [covers: ITEM-6] file: `src-app/ui/tests/e2e/visual/approval-actions-reachable.spec.ts` — asserts: at 390x844, in BOTH themes, the approval card's TOOL NAME has a non-zero and legible RENDERED width (not merely a non-empty `textContent`) — pre-fix it measured `width=0` against a `scrollWidth` of 98 while the two `whitespace-nowrap` secondary labels took the full 238px row. Deliberately asserts rendered geometry rather than text presence: the pre-existing TEST-11 asserts `toContainText('get_forecast')` and passed for the entire life of the defect, which is exactly how a consent surface shipped unable to say what it was asking consent for. It then drives a 64-character HOSTILE name (the fixture's 12-char name fits trivially and could never fail) and, critically, asserts the CEILING as well: under a 6400-character name the identity region stays clamped and the card stays within the viewport. That second half exists because round 3 fixed the truncation by removing the BOUND, which measured a 5123px card with Deny ~2800px below the fold — and because TEST-8's first two assertions are both SATISFIED by unbounded growth (a wrapping box never overflows horizontally, and one of them requires the box to grow), so they cannot stand alone.
- **TEST-9** (tier: e2e) [acceptance] [invariant: INV-2] [covers: ITEM-1] file: `src-app/ui/tests/e2e/visual/approval-actions-reachable.spec.ts` — asserts: a NARROW CONTAINER at a WIDE viewport is protected — with the card squeezed to 260px at a 1280x900 viewport, every decision control stays unclipped and pressable and the row wraps. Added during the fix round because the blind audit found the primitive's central design claim untested: the rule is content-driven precisely BECAUSE a card's container width is independent of the viewport (split panes, side panels, an indented virtualized row), yet every other test drove the viewport — the one axis the design argues you must not rely on.
- **TEST-10** (tier: e2e) [acceptance] [invariant: INV-4] [covers: ITEM-6] file: `src-app/ui/tests/e2e/visual/approval-actions-reachable.spec.ts` — asserts: at 390x844 in BOTH themes the attacker-controlled SERVER LABEL (the card's trust anchor, uncapped by `mcpServerParenLabel`) is disclosed WITHOUT clipping AND cannot push the decision row off screen — its box carries no horizontal overflow, and under a ~930-character hostile label the identity region stays clamped, the card stays within the viewport, and Deny stays pressable. Added in round 4: the round-3 change to this label shipped with ZERO coverage (reverting it left every other test green), and round 3's own fix for it was unbounded, which measured a 931px card pushing Deny to y=805 in an 844px viewport.
