...
## Port hygiene — properties of the DIFF, not units of work

**Amended after phase 8 (see DEC-13 and DRIFT-1.10).** These were originally ITEM-7 and
ITEM-8. That was a modelling error: the phase-3 gate requires every ITEM to be covered
by a TEST, and these two are not implementable work with observable behaviour — they
are assertions ABOUT the resulting diff, whose only honest proof is a command. Rather
than invent a hollow test to satisfy the bipartite mapping, or mark them `[DESCOPED]`
(which would be false — they were done), they are recorded here as binding release
conditions and verified by command in TEST_RESULTS.md's *Controls* section.

- **H1**: strip from every pick anything that does not belong on paws main — all
  `.lifecycle/**` paths, and the stray OpenAPI output under `src-app/server/ui/` that
  `abc8d2429` committed (upstream itself deleted it in `88081b800`).
- **H2**: the branch moves **no** submodule gitlink (`sdk`, `agent-kit`, `pgvector`),
  touches no UI workspace, and adds **no** `GRANDFATHERED` entry to
  `migration_immutability.rs`.
...
out-of-scope paths". H1 is the rule that enforces this.
...
wrong-path artifact upstream itself deleted in `88081b800`. Dropped by H1.)
...
- **ITEM-5** — verdict: CONCERN — the only item with a migration and an `agent-core` change. Concern is procedural, not semantic: `abc8d2429` edits the migration `ee48f1a77` adds, and also commits 58k lines of stray `src-app/server/ui/` output. Resolved by squashing (DEC-3) and by hygiene rule H1 (DEC-5). Not BLOCKED.
