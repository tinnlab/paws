...
### DEC-15: The blind audit flagged ITEM-6 (deriving `isAuthenticated` from a persisted token) as a trust-boundary widening. Keep it, narrow it, or cut it?
**Resolution:** CUT it. `loadContext.ts` is restored byte-for-byte; `liveSession.ts` and its spec are deleted; a new TEST-14 asserts the widening stays out.
**Basis:** codebase + measurement. Three independent audit angles (security ×2, design-conformance) raised the same three problems: (a) a REVOKED-but-not-yet-expired token in `localStorage` would deliver and `initialize()` every `ctx.isAuthenticated`-gated module, and `loader.ts` documents that modules are NEVER unloaded, so the later failed verification cannot undo it; (b) `expiresAt: null` is not only a legacy shape — `seedSessionTokens` writes it whenever a mint response omits `expires_in` — so an arbitrarily old token would qualify; (c) it applies to the DESKTOP bundle too, directly contradicting `bootSessionVerify.desktop.ts`'s own stated reason that a persisted desktop token is stale by construction. Decisive factor: re-measuring the boot probe with the widening removed showed **no change to any metric it was meant to move** (`/api/auth/me` still starts at 46–60 ms; identical request totals) — the entire waterfall improvement comes from ITEM-5. A security-adjacent widening with zero measured benefit is not a trade worth making.

- DESCOPED: ITEM-6 — deriving `isAuthenticated` from a persisted token widens the authenticated-tier trust boundary for a revoked token, contradicts this branch's own desktop reasoning, and measured ZERO benefit once ITEM-5 landed (see DEC-15) [approved: blind-audit finding (security ×2 + design-conformance angles) + re-measurement, 2026-07-26]
