...
- **TEST-6** (tier: unit) [covers: ITEM-5] file: `src-app/ui/src/modules/chat/core/stream/ChatStreamClient.subscription.store.test.ts` — asserts: a REJECTED subscription `fetch` (the CORS-preflight-refusal shape, not a status) drops the connection id and aborts the live stream so the connect loop re-PUTs; that after the consecutive-failure limit `onSubscriptionError` is called, and for a non-2xx too; that under a PERMANENT failure it keeps being able to report (strictly more than once over 14 cycles) while staying far below one report per attempt; that scoping the stream again — what the store does at the start of every turn — re-reports immediately, on BOTH the same-conversation and different-conversation paths; that a blip SHORTER than the limit stays silent; that recovery is reported once a subscription to a real conversation succeeds, and NOT for a bare `conversation_id: null` unsubscribe; that a 204 reports nothing; and that the message emitted is the exported constant the clear path matches on.
- **TEST-7** (tier: unit) [covers: ITEM-6] file: `src-app/ui/src/modules/chat/core/stores/chat/subscriptionError.store.test.ts` — asserts: the action raises the existing `error` banner (the one `ConversationPane` renders as `chat-conversation-error-alert`) and touches NOTHING else — `sending`, `isStreaming`, `streamingMessageId` and `lastTurnInterrupted` are all left exactly as they were, in both the mid-turn and at-rest states, so a reply that completed normally is never decorated as interrupted and a turn is never reset from under `sendMessage`; that the banner text is non-blank and names the remedy; that a live delivery failure replaces a stale earlier error; and that the recovery action clears the banner this feature raised — matched against the client's EXPORTED constant, not a re-spelled copy — while leaving an unrelated error alone.
...
- **TEST-11** (tier: e2e) [acceptance] [invariant: INV-4] [covers: ITEM-5, ITEM-6] file: `src-app/ui/tests/e2e/chat/chat-stream-subscription-failure.spec.ts` — asserts: with `PUT /api/chat/stream/subscription` aborted at the network boundary (the preflight-rejection shape — a rejected request, not a status), opening a conversation is SURFACED rather than silent: `chat-conversation-error-alert` is visible with non-empty text matching /live updates/ AND /reload/, the client is shown to have RETRIED (>=3 attempts, so the reconnect path really ran), the banner is still visible after a settle window rather than flashing, and the composer + send button stay enabled; a positive control in the same spec proves an unaborted subscription raises NO banner and that the real PUT is issued with the connection-id header and accepted, so the assertion cannot pass by a banner that always appears.
...
4. The INV-4 acceptance test asserts what the branch actually promises — the
   failure is SURFACED with a remedy, after real retries, and persists — paired
   with a positive control proving a healthy subscription stays silent. It
   deliberately does NOT assert "the spinner stops": the fix surfaces the failure
   and does not terminate a turn's UI state (see the design doc's INV-4), and a
   spinner assertion in this spec would also be vacuous, since it never starts a
   turn.
