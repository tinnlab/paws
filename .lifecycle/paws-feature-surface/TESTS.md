...
- **TEST-6** (tier: integration) [acceptance] [invariant: INV-6] [covers: ITEM-10] file: `src-app/server/tests/paws_surface/mod.rs` — asserts: every permission gate still behaves as before — a user holding a permission still succeeds (positive control) and an unauthenticated request is still refused. Nothing is loosened. **Amended in round 1**: the original wording also asserted the DEC-4 revokes denied; those revokes were withdrawn (they broke chat for non-admins), so `test_hidden_features_keep_their_grants` now pins the opposite — the grants REMAIN, deliberately.
...
- **TEST-14** (tier: e2e) [positive-control] [covers: ITEM-5, ITEM-10] file: `src-app/ui/tests/e2e/17-paws-surface/project-references-absent.spec.ts` — asserts: an ADMIN (holding `*`, so the strongest possible subject for an absence claim) creates a project, opens its DETAIL page, and sees its surviving "Knowledge files" knowledge kind PRESENT (the positive control — this is what makes "absent" mean "removed" rather than "never rendered"), while "References" and "Knowledge bases" are absent. **Amended in round 1**: the `[negative-perm]` framing is dropped because the DEC-4 revokes were withdrawn — the entries are gone because the module is hidden, not because a permission is missing, and no new permission is introduced by this branch. The original also asserted on the project LIST page, where knowledge kinds never render at all.
