...
- **ITEM-7** — verdict: CONCERN — (iteration) rendering the injected result as a SYSTEM/observation turn is BLOCKED on non-trivial new shared-pipeline plumbing (both turn-start paths hardcode `MessageRole::User`; `MessageRole` has no observation role; System messages are dropped from the LLM context). Resolved by a human-approved `[DESCOPED]` disposition (DECISIONS.md) + a BLOCKED report to the coordinator; the user-role interim is retained. Not a hard BLOCK of the branch — the feature ships without it.
