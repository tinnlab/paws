...
- **TEST-23** (tier: integration) [covers: ITEM-17] file: `src-app/server/tests/mcp/tool_call_redaction_test.rs` — asserts: a tool invoked with a secret-bearing argument stores `[redacted]`, and the detail endpoint never returns the raw value.
