...
- **TEST-38** (tier: e2e) [covers: ITEM-17] file: `src-app/ui/tests/e2e/chat/ask-user-stringified-schema.spec.ts` — asserts: with a REAL configured LLM (resolved through the shared `TEST_LLM` seam, so ANY configured vendor or local bridge satisfies it — never a vendor-specific key gate), an under-specified request still renders an `ask_user` form with real fields and NOT the degraded no-fields card, i.e. the decode path introduced here does not regress the real-model flow. Retried like its sibling real-LLM control specs. **AMENDED at phase 8 (see DEC-21):** enumerated here as a `test.skip(!TEST_LLM, …)` conditional gate, it is implemented as an UNCONDITIONAL `expect(TEST_LLM, NO_LLM_SKIP).toBeTruthy()` precondition. A missing LLM is a missing DEPENDENCY, not the platform-incompatibility that is the only legitimate skip, and a skipped real-LLM leg is counted as covered while proving nothing. RUN against the local Qwen bridge, not skipped.
