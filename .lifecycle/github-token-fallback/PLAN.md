...
- **ITEM-2**: Add a pure classifier `is_auth_rejection(status) -> bool` in `engine/download.rs` implementing DESIGN §4's table: **401 ⇒ true; everything else, including every 403, ⇒ false**. Status ONLY — never the headers, never the body, never the token string (INV-3). (Amended at FIX_ROUND-1: the header-based 403 split was unsound — see DEC-1.)
...
- **ITEM-12**: (added at FIX_ROUND-1) Make `ANONYMOUS_RETRY_LIMIT` load-bearing rather than decorative — derive `authenticated` from it so the constant is the SINGLE mechanism bounding the fallback, and pin it with a test that rejects the anonymous re-issue too (the only path where the bound is observable).
...
- **ITEM-12** — verdict: PASS — collapses two parallel conditions into one; audited as dead code in round 1 (deleting the constant changed nothing and no test noticed).
