...
## FIX_ROUND-4 additions

- **TEST-21b** (tier: unit) [covers: ITEM-4, ITEM-5] file: `src-app/server/src/modules/chat/stream/registry.rs` — asserts: the chat twin of TEST-21 — the user-scoped sweep repairs an orphaned `by_user` entry, and with a LIVE connection also present the entry SHRINKS rather than vanishing, with `clients` left untouched. The repair branch had been duplicated into this registry untested.
- **TEST-19a / TEST-20a** (tier: integration) [covers: ITEM-1, ITEM-2] files: `src-app/server/tests/{sync/subscribe_test.rs,chat/stream_slot_reclaim_test.rs}` — asserts (as the non-vacuity anchor inside TEST-19/TEST-20): a `HEAD` request genuinely reaches `register()`. With the user's whole allowance held open by LIVE streams, a HEAD must be refused `429` — which is only possible if it was routed to the handler and attempted a registration. Without this, both HEAD tests would pass vacuously if HEAD were ever short-circuited before the handler.
