...
## Unit — regressions found by the blind audit (ITEM-3, ITEM-4, ITEM-5)

- **TEST-18** (tier: unit) [covers: ITEM-3] file: `src-app/ui/src/modules/chat/core/stores/chat/sendMessage.store.test.ts` — asserts: a pre-flight abort clears the latched branch-fork fields DIRECTLY (not via the lazy `clearPendingBranch` action, which would itself fail in the stale-build case and also clears `editingMessage`), and the next healthy send carries no `create_branch_from_message_id` — so an aborted REGENERATE cannot make the user's next message silently fork at a stale assistant anchor.
- **TEST-21** (tier: unit) [covers: ITEM-3] file: `src-app/ui/src/modules/chat/core/stores/chat/sendMessage.store.test.ts` — asserts: the mirror image of TEST-18 — an aborted EDIT keeps `editingMessage` AND its fork anchor, and the retry still branches from the edited message. This is the regression TEST-18's first implementation nearly shipped — the edit flow latches the same fork fields but the user stays in edit mode, so clearing them turns the retry into an appended turn.
- **TEST-19** (tier: unit) [covers: ITEM-4] file: `src-app/ui/src/modules/chat/core/stores/chat/sendMessage.store.test.ts` — asserts: an empty `branch_id` (what `conversation.active_branch_id || ''` produces, since the generated type makes it optional while the server declares it a `Uuid`) is never POSTed.
- **TEST-20** (tier: unit) [covers: ITEM-4] file: `src-app/ui/src/modules/chat/core/stores/chat/sendMessage.store.test.ts` — asserts: an extension-contributed `branch_id` still WINS over the conversation's, preserving the precedence of the old `{ id, branch_id, ...allRequestFields }` literal.
...
- **TEST-14** (tier: e2e) [covers: ITEM-5, ITEM-6] file: `src-app/ui/tests/e2e/chat/send-field-composition-failure.spec.ts` — asserts: REPEATED failed sends (4 in a row) issue ZERO `POST …/messages` between them, and then the recovery the error message PRESCRIBES actually works — once the blip clears, a reload restores sending and the POST carries a `model_id`. It deliberately does NOT assert "the dispatcher re-attempts the import": the negative control disproved that as an e2e-layer discriminator (it passed with the old memoize-forever policy reinstated, because the per-pane store and its dispatchers do not necessarily survive the failure re-render), so the claim was removed rather than kept hollow. The never-memoize property is pinned at the unit layer by TEST-6/TEST-6b, both verified red under that mutation.
