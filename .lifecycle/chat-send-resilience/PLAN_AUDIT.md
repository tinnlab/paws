...
- **ITEM-9** — verdict: PASS — added during phase 5 (DRIFT-1.8) after the e2e caught it: `autoWarmLazyActions`'s `try/catch` only ever caught a SYNCHRONOUS throw while `preload()` returns a promise, so a failed warm-up chunk became an unhandled rejection — an uncaught page error for work nobody awaited. One-line `.catch` at the single call site, at debug volume; no behaviour depends on the warm-up succeeding. Squarely inside the design's Rank 2 ("no dynamic-import failure recovery anywhere"), so in scope rather than creep.
