...
- **TEST-11** (tier: e2e) [covers: ITEM-3, ITEM-7] file: `src-app/ui/tests/e2e/chat/composer-picker-popover.spec.ts` — asserts: with **26 seeded assistants**, the assistant panel's measured height stays within the declared cap (the SAME bound TEST-12 holds the KB panel to), its list has a scrolling ancestor whose `scrollHeight > clientHeight`, and the LAST ROW IN THE DOM is GEOMETRICALLY outside the scroller's box before scrolling and inside it after. (The row is picked by DOM position, not by seeded name: the store returns assistants newest-first, so the last-seeded name is actually the FIRST row — an earlier draft asserted on it and went red.) (`toBeVisible()` alone is true for a row clipped inside a scroller, so reachability is asserted with bounding-box containment, not visibility.)
- **TEST-12** (tier: e2e) [acceptance] [invariant: INV-2] [covers: ITEM-3, ITEM-8] file: `src-app/ui/tests/e2e/chat/composer-picker-popover.spec.ts` — asserts: with **26 seeded knowledge bases** the KB popover panel's `boundingBox().height` stays at or under the declared cap (`MAX_PANEL_HEIGHT` = 360px: the 256px list + the search box + panel padding + the popup ring) even though 26 rows exist, the scrolling element carries `data-overlayscrollbars-viewport` (the app's overlayscrollbars, NOT a native scrollbar), and the last row in the DOM is geometrically outside the scroller's box before scrolling and inside it after. Fails if the list is unbounded (no overflow) or if a native scroller replaced `ScrollArea` (attribute absent).
...
- **TEST-18** (tier: e2e) [covers: ITEM-9] file: `src-app/ui/tests/e2e/chat/composer-picker-popover.spec.ts` — asserts: the assistant and knowledge-base trigger ROWS render with identical shared-row metrics — computed padding, font-size AND leading-icon size (all three measured) — each exposes an accessible name, and BOTH are held to `PlusMenuItem`'s declared metric ABSOLUTELY (`6px` block / `12px` inline padding, `14px` label, `16px` icon). Absolute rather than sibling-compared: the assistant-vs-kb equality is true by construction once both use the shared row, and the one always-present sibling carries its testid on the kit `Upload` ROOT (padding `0px`), so comparing against it measured the wrong node — which is how the earlier draft went red.
