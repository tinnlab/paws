...
- **TEST-13** (tier: e2e) [acceptance] [invariant: INV-1] [covers: ITEM-2] file: `src-app/ui/tests/e2e/chat/composer-picker-popover.spec.ts` — asserts: after seeding an assistant with a 200-character name alongside short-named ones, the popover panel's `boundingBox().width` is ≤ 320px AND the long row's label element has `scrollWidth > clientWidth` (it is truncated) AND the panel width equals the width measured with only short names — i.e. the long name did not widen the panel. Fails on the current `main` behaviour, where the panel grows to fit.
