...
- **ITEM-12**: [DESCOPED — the `sdk` half] Reconcile generated registries that are ALREADY stale on `origin/main`. Discovered at phase 5: `npm run check` fails on a pristine `origin/main` worktree at `check:testid-registry` AND `check:state-matrix`. The owner is fixing main's drift separately, so the **`sdk`/testIds half is DROPPED from this branch** (the submodule pointer is back at `origin/main`'s `70576db7`; the branch touches no SDK file). The **gallery state-matrix half is NOT separable and stays**: this feature adds a surface (`ComposerPickerPopover`) and deletes one (`AssistantSelector`), so `stateMatrix.generated.ts` MUST be regenerated, and regeneration is all-or-nothing — it necessarily also picks up main's 4 drifted surfaces (`AttachCitationsDialog`, `ProxyTokenCard`, `McpServerRuntimeTab`, `WorkflowMetadataDialog`) and drops a deleted one (`OnboardingRedirect`), whose 6 `stateCoverage` keys are then load-bearing for `tsc` (the manifest is `satisfies Record<RequiredState, …>`). Reverting them would not restore main's state — it would break the build.
...
- **ITEM-12** — verdict: CONCERN — this is main's drift, not the feature's. Resolution (owner-directed): the separable half (the `sdk` testIds registry, which WAS its own commit) is dropped from the branch entirely; the inseparable half (the gallery state matrix) stays, with the 6 orphaned `stateCoverage` keys carrying an explicit "pre-existing, reconciled only to unstick the gate" reason rather than invented coverage. Consequence recorded honestly rather than papered over: `check:testid-registry` is now RED on this branch — identically to base, since the branch no longer touches that file. It goes green after main's fix lands and the branch is rebased + regenerated.
