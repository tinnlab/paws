...
## Verdicts for the items added after the blind audit (phase 5/7 amendments)

- **ITEM-14** — verdict: PASS — `pdfjs-body.tsx:46` is the 6th residual instance, found by the real lint rather than by the hand tally in DESIGN §5 (DRIFT-1.1). The fix is the repo's own component-per-case idiom (`7bb34e223`), and it strictly improves the file: the wrapper makes ALL ~12 of the body's hooks unconditional, not just the flagged proxy read. `props` is referenced only on the three guard lines, so the split is mechanical. Covered by the real-`PdfJsBody` gallery e2e (TEST-14).
- **ITEM-15** — verdict: CONCERN — widening the scanned roots to `sdk/packages` means a violation inside the **submodule** can fail ziee's `npm run check`, and DEC-2 records that the submodule is pinned to a branch not pushed this round. Accepted because (a) those components render inside BOTH apps, so an O1/O2 defect there crashes both, (b) `sdk/packages/framework/src/stores.ts` is where the proxy that makes H2 necessary lives, and (c) it measures 0 findings today across 251 files. The scope change is recorded as DEC-14 rather than left as a silent widening; if it ever becomes a friction point the root is one list entry.
- **ITEM-16** — verdict: PASS — pure hardening of the gate's own failure modes, all traceable to a confirmed audit finding: a bad `--root`, an unknown flag, a zero-file scan, a broken proxy registry and a crash could each previously exit 0 or reuse the "violations found" code, and the byte-identity guard lived only outside the gate. No rule semantics change. The distinct exit code 2 (DEC-15) is additive: `check` still treats any non-zero as failure.
