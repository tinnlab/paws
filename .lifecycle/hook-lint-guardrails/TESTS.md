...
- **TEST-15** (tier: unit) [covers: ITEM-15, ITEM-16] file: `src-app/ui/scripts/lint-hooks.test.mjs` — asserts: the shared SDK packages are in scope (>100 files scanned, 0 findings) and both workspace copies resolve an IDENTICAL root set; `registryHealthError()` trips on a zero-file scan and on a proxy registry below its floor and is null when healthy, with the live registry more than double the floor; every unusable `--root` (missing dir, a file, a bare `--root`, an unknown flag) exits **2** — distinct from 0 (clean) and 1 (violations); `parseArgs` accepts repeatable `--root=` and the `--root <dir>` space form; and `siblingDriftError()` is null with `main()` consulting it, so the byte-identity guard runs inside the gate rather than only in a test.
