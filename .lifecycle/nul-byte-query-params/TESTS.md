...
- **TEST-9** (tier: integration) [covers: ITEM-4] file: `src-app/server/tests/chat/nul_query_param_test.rs` — asserts: **(a)** `GET /conversations?search=%00` → **400** `VALIDATION_ERROR`; **(b)** an extra input shape — `GET /conversations?search=%5C%00` (a NUL alongside a LIKE metacharacter) is ALSO **400**. NOTE: this was originally billed as an ordering proof; that claim is WITHDRAWN (FIX_ROUND-1) because `escape_like` neither removes nor introduces a NUL, so both orderings reject the same inputs and no query string can distinguish them; **(c) happy-path counterpart**: `GET /conversations?search=<seeded title substring>` → **200** with the seeded conversation present; **(d) ownership control**: a second user searching the same term → **200**, `total == 0`.
...
- **TEST-19** (tier: integration) [covers: ITEM-10] file: `src-app/server/tests/chat/nul_query_param_test.rs` — asserts: **(a)** `GET /conversations/{id}/messages/search?q=%00` → **400** `VALIDATION_ERROR`; **(b) happy-path counterpart**: TWO real messages are seeded (one matching) and `?q=roadmap` → **200** with `total == 1` and the snippet carrying the hit — discriminating, so an endpoint ignoring `q` fails; **(c) ownership control**: a second user hitting the same conversation id → **404** (the endpoint resolves the conversation owner-scoped before reading `q`).
...
## Added in FIX_ROUND-1 (blind audit)

- **TEST-23** (tier: unit) [acceptance] [invariant: INV-2] [covers: ITEM-14] file: `src-app/server/src/common/text_guard.rs` — asserts: `guard_raw` returns every valid input (incl. `""`, `"   "`, `"  padded  "`) byte-for-byte unchanged, and that the two entry points DIFFER exactly on blank (`normalize_text_filter("") == None` vs `guard_raw("") == Some("")`) — the property whose absence caused the widening regression.
- **TEST-24** (tier: integration) [covers: ITEM-14] file: `src-app/server/tests/background_mcp/nul_query_param_test.rs` — asserts: `GET /background/runs?status=` and `?kind=` return **0** rows, never the unfiltered total — an empty filter value must stay a filter.
- **TEST-25** (tier: integration) [covers: ITEM-14] file: `src-app/server/tests/mcp/nul_query_param_test.rs` — asserts: `GET /mcp/tool-calls?tool_use_id=` returns `total == 0`, not the caller's whole history.
- **TEST-26** (tier: integration) [covers: ITEM-14] file: `src-app/server/tests/llm_local_runtime/nul_query_param_test.rs` — asserts: `GET /local-runtime/versions?engine=` returns an empty list, not every version of every engine.
- **TEST-27** (tier: integration) [acceptance] [invariant: INV-4] [covers: ITEM-2] file: `src-app/server/tests/common/nul_query_param_sweep_test.rs` — asserts: at the HTTP tier, a non-NUL control character (`\n`/`\t`/ESC/DEL) inside a GUARDED filter (`/projects?search`, `/memories?search`, `/mcp/servers?search`) still returns **200**. Replaces an inert control that probed two parameters the diff never touched.
- **TEST-28** (tier: unit) [acceptance] [invariant: INV-3] [covers: ITEM-8] file: `src-app/server/src/common/text_guard.rs` — asserts: the rejection MESSAGE format (`{field} cannot contain NUL characters`) for four field names. Replaces the status/error-code comparison, which every pre-existing private copy already satisfied and so could not detect a re-fork.
- **TEST-29** (tier: unit) [covers: ITEM-8] file: `src-app/server/src/modules/project/handlers.rs` + `src-app/server/src/modules/user/handlers/groups.rs` — asserts: each module's own `reject_nul` wrapper renders that shared message format, pinned where the wrapper lives (no feature-module imports into `common/`).
- **TEST-30** (tier: integration) [covers: ITEM-15] file: `src-app/server/tests/common/nul_query_param_sweep_test.rs` — asserts: the BODY-path members found by the post-audit live probe (`POST /assistants {description,instructions}`, `POST /conversations {title}`, `POST /knowledge-bases {description}`, `POST /memories {content}`) each return **400** `VALIDATION_ERROR`, each with a 200/201 happy-path counterpart in the same test.

| ITEM | covered by |
|---|---|
| ITEM-14 | TEST-23, TEST-24, TEST-25, TEST-26 |
| ITEM-15 | TEST-30 |
| ITEM-16 | TEST-31 (the regen-parity check in TEST_RESULTS) |

- **TEST-31** (tier: unit) [covers: ITEM-16] file: `src-app/server/src/openapi/emit_ts.rs` — asserts: the committed `types.ts` regenerates byte-for-byte from the committed `openapi.json` (the repo's existing golden parity test), which is what proves the regen for both workspaces was actually run and committed.
