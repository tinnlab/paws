...
and TEST-12 satisfy the frontend gate.
...
- **TEST-11** (tier: e2e) [covers: ITEM-5, ITEM-12] file: `src-app/ui/tests/e2e/14-voice/voice-model-mgmt.spec.ts` — asserts: when a model install fails, the row renders an explicit labelled "Install failed" message with the reason and an enabled Retry control, in BOTH the models card and the runtime-versions card — never a bare unlabelled secondary line.
...
| ITEM-10 | TEST-12 |
| ITEM-11 | TEST-13 |
| ITEM-12 | TEST-11, TEST-12 |

| INV | Pinned by (`[acceptance]`) |
|---|---|
| INV-1 | TEST-9, TEST-12 |
| INV-2 | TEST-12 |
| INV-3 | TEST-3 (no artifact left on any failure exit) + TEST-7 (upload arm) — see note |
| INV-4 | TEST-5 |
| INV-5 | TEST-7 |
| INV-6 | TEST-6 |
| INV-7 | TEST-8 |
