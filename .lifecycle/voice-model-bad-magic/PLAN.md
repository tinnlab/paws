...
- **ITEM-13**: Gate the new Retry control on `voice::admin::manage` inside `DownloadFailureRow` (so both cards inherit it), leaving the failure message ungated; and finish INV-6's catalog-side suppression on `AvailableVersionsCard` (`size_bytes > 0`), which fix-round 1 applied to the models card only. Realizes INV-8 and closes the last ITEM-12 drift. *(Added in fix-round 4 — see DRIFT-2.9 / FIX_ROUND-4.)*
