...
- **ITEM-13** — verdict: PASS — **audited retrospectively**: this item did not exist at Phase 2; it was added to PLAN.md during fix-round 4 in response to DRIFT-2.9, so its verdict is recorded here after the fact rather than before implementation. Audited on the same criteria as the rest. Two halves, both narrow: (a) the Retry gate — `DownloadFailureRow` wraps only the Retry control in `<Can permission={Permissions.VoiceAdminManage}>`, the identical constant and idiom already gating the Install button it re-issues (`AvailableModelsCard.tsx:80/115/257`, `AvailableVersionsCard.tsx:207`), so this introduces no new permission and no new gating mechanism; the failure MESSAGE stays ungated, which is correct because both cards' task lists are served under `voice::admin::read` — a read-only voice admin is entitled to see why a task failed, they just must not be handed a control that can only 403. Gating inside the shared component rather than at the two call sites is what makes the two cards unable to drift apart, consistent with ITEM-12's rationale. (b) the `size_bytes > 0` catalog-side suppression on `AvailableVersionsCard.tsx:186` — a one-condition change that finishes on the versions card what fix-round 1 applied to the models card; INV-6 is stated about the page, so leaving one card unguarded left the invariant only half-true. No schema, API, or permission-model change. The one thing worth flagging: a permission gate is invisible to a test that runs as an admin, which is exactly why TEST-16 drives the deny leg as a `voice::admin::read`-only user under the `no-403` fixture — without that test this item would be unfalsifiable.

No `BLOCKED` verdicts. ITEM-13 is the sole retrospectively-audited item; every other verdict above predates implementation.
