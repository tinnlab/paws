...
- **TEST-9** (tier: e2e) [covers: ITEM-8, ITEM-2] file: `src-app/ui/tests/e2e/visual/store-case-collision.spec.ts` — asserts: in a real browser against the backend-free gallery, (a) every relocated store, imported through its **bare directory specifier** so Vite runs the same extension-probe-then-`/index` walk the compiler does, is the STORE module and not its sibling component — proven per store against that component's own exports, with a per-store control that the component exposes ≥1 function; and (b) each overlay-backed store OPENS its drawer through the store's own action, with the opened drawer asserted to be the RIGHT one. Runtime complement to `tsc`, which proves specifiers resolve but not what they resolve TO.
- **TEST-10** (tier: unit) [covers: ITEM-11, ITEM-12] file: `src-app/ui/scripts/lint-case-collisions.test.mjs` — asserts: TEST-2(b) — on a synthetic `AgentStepForm.tsx` + `agentStepForm.ts` fixture (the exact shape of the 25th collision) the guard exits **1** and reports it as `file-vs-file`, naming both files. A guard that only knows the file-vs-directory rule exits 0 here, which is precisely how the 25th collision survived phase 5.
- **TEST-14** (tier: unit) [acceptance] [invariant: INV-1] [covers: ITEM-1, ITEM-11, ITEM-14] file: `src-app/ui/scripts/lint-case-collisions.resolution.test.mjs` — asserts: for EVERY real import specifier in both workspaces (~10k), resolution performed twice — once against the real case-sensitive filesystem, once through a case-INSENSITIVE sibling lookup in TypeScript's probe order — yields the SAME module. This is the behavioural anchor: it detects divergence by OUTCOME, so unlike the shape-matching guard it is not blind to shapes nobody thought of (including the desktop resolver's cross-tree Tier-1 probe, which no sibling rule can see). Carries an anti-vacuity floor (>5000 specifiers examined, >90% resolving) and a CONTROL proving the simulator reproduces both real bug shapes. Verified end-to-end: reintroducing the exact 25th collision turns it RED naming `linux=AgentStepForm.tsx` / `macos=agentStepForm.ts`.
- **TEST-15** (tier: unit) [covers: ITEM-5, ITEM-12, ITEM-14] file: `src-app/ui/scripts/lint-case-collisions.test.mjs` — asserts: the guard, invoked EXACTLY as `npm run check` invokes it (no args, its own DEFAULT roots, in a synthetic repo with the real script at the real relative path), (a) passes clean, (b) BLOCKS on a collision buried five levels down inside `components/` — where the 24 real ones lived, (c) REPORTS but does not block an sdk-only collision and does not then print the unqualified green line, (d) blocks when both are present. Every other RED fixture uses `--root=`, a different code path pointed at a flat temp dir; this is the only test that exercises the default-root classification. Verified by mutation: it kills the "decouple blocking from the advisory label" mutation that previously let a planted collision in `src-app/ui/src` exit 0 with the whole suite green.
- **TEST-12** (tier: unit) [covers: ITEM-1, ITEM-12] file: `src-app/ui/scripts/lint-case-collisions.test.mjs` — asserts: the durable half of the convention claim, true on this branch AND on main — every store directory under a `stores/` ancestor is a real store, and `stores/` is the DOMINANT convention, expressed as a RATIO (`under > outside × 2`) rather than a magic count. A threshold like `>= 110` would go red the day someone legitimately consolidates six stores, with a message about case collisions that had nothing to do with it. The branch-scoped half lives in the provenance suite (TEST-6/TEST-7), outside `check`.
- **TEST-13** (tier: e2e) [covers: ITEM-8, ITEM-13] file: `src-app/ui/tests/e2e/visual/store-case-collision.spec.ts` — asserts: the 12 overlay identity markers are MUTUALLY EXCLUSIVE (no duplicate, none a substring of another). Without it the per-overlay assertions can drift back into markers several drawers satisfy — the earlier `/group/i` matched three drawers and `/provider/i` two, so **8 of 10 adversarial cross-pairings passed**. Verified out-of-band after the fix: **0 of 132 wrong pairings pass, 12 of 12 correct ones do**.
- **TEST-11** (tier: unit) [covers: ITEM-12] file: `src-app/ui/scripts/lint-case-collisions.test.mjs` — asserts: TEST-2(c) — `ProviderGroupAssignmentCard.desktop.tsx` beside `providerGroupAssignmentCard/` exits **1**, because the desktop resolver probes the `.desktop` infix ahead of the directory; and TEST-2(f) — a missing root, and an unreadable directory containing a real collision, each exit **1** with `FATAL` instead of printing a green line. Fail-open was the guard's actual prior behaviour, reproduced by a blind auditor.
...
| ITEM-11 (the 25th collision) | TEST-1, TEST-10, TEST-8 |
| ITEM-12 (widened, fail-closed guard) | TEST-1, TEST-10, TEST-11, TEST-2, TEST-12 |
| ITEM-13 (advisory sdk + provenance split + identity markers) | TEST-3, TEST-11, TEST-12, TEST-13 |
| ITEM-14 (behavioural anchor + simplification) | TEST-14, TEST-15, TEST-1, TEST-3 |
