...
- A11 [G] **element border clipped by ancestor/container** — a bordered box whose border pixels are cut on a side because its border-box reaches/exceeds a NON-scrolling clipping ancestor's (overflow hidden/clip) content edge. A3 (protrusion) deliberately skips clipped elements; A11 fills that gap. *(user miss #18: tool-call card top/left/right borders clipped)*
- A13 [G] **child block breaks the parent's alignment axis** — in a RIGHT-aligned message (the whole bubble pushed right via `align-self: flex-end` / `margin-left: auto`), a child content block (attachment/file list, action row, metadata) that stays LEFT-packed floats at the far left of the message's own width, detaching from the bubble it belongs to. Flag a file/attachment/image block whose right edge sits >40% of the message width away from the message's right edge while its left edge hugs the message's left — an alignment mismatch among ONE message's children. Only right-aligned messages are inspected (left-aligned assistant messages legitimately pack left). *(a user message with an attached file: the file card renders left-aligned under the right-aligned bubble.)*
- A14 [G] **dead space from an over-tall min/fixed height** — a container with an explicit `min-height` / fixed `height` whose CONTENT (the union extent of its visible children) fills far less than the box leaves a large blank band. Flag when `box − content` exceeds BOTH 35% of the box AND a ~120px floor, and the content is NOT merely scroll-clipped (`scrollHeight ≤ clientHeight`). *(the inline file-viewer body `h-[min(360px,55vh)]` holding a 2-3-row table leaves a big empty band below the rows.)*
- A12 [G] **cramped nested borders** — a bordered edge-adjacent control (outline button/input) whose border sits within a small gap (<~8px) of a bordered ancestor's inner edge, creating a crowded double-border. Fix is usually variant (edge-adjacent action → ghost, not outline). *(user miss #21c: edit-conversation-title / edit-llm-provider-name outline buttons crammed against container borders)*
...
## O. React runtime correctness (Rules of Hooks)
- O1 [L] **conditionally-evaluated hook call** — a `use*()` call that is only reached on SOME renders (a ternary branch, the right-hand side of `&&`/`||`/`??`, an `if`/`else` body, a loop body, a `switch` case, a `catch` body), so the hook COUNT varies with the condition and React throws "Rendered more hooks than during the previous render" the moment it flips. ACTIVE source lint, gating (exit 1). The canonical instance is `usePermission(A) || usePermission(B)` — `||` short-circuits the SECOND hook whenever the first is truthy — which shipped in 13 settings sections (file-rag/memory/summarization) and crashed on every permission-state flip (fixed in 649ae7180). Fix: call every hook unconditionally and combine the RESULTS. Deliberately does NOT flag hooks after an early return (the type-guard idiom — 5 such calls across 3 legacy components — and the standard `rules-of-hooks` rule's territory). Covers both `useX()` and `React.useX()`. Detector: `scripts/lint-hooks.mjs` (`npm run lint:hooks`). *(Not catchable by Biome: `useHookAtTopLevel` is not enabled here.)*
- O2 [L] **conditionally-evaluated store-proxy read** — a reactive store-proxy field read (`SomeStore.field`, or `const { … } = SomeStore`) reached on only SOME renders. In this codebase a proxy read **IS a hook**: path 4 of `createStoreProxy` calls `useEffect` + `useStore(useShallow(...))` (`sdk/packages/framework/src/stores.ts`), so it carries exactly the O1 hazard while looking like a plain property access — no off-the-shelf rule can see it. Shipped as a crash on opening the edit-model drawer (`modelId ? LlmProvider.providers… : null`, fixed in 57f9fdb5b). Covers all seven conditional contexts INCLUDING after an early return. Never flags the hook-free paths: the `$` snapshot, `__setState`/`__refCount`/`__refTracker`/`__destroyed`, or an ACTION (called or passed by reference; actions are tracked PER PROXY so a name that is an action on some other store does not exempt a real state field). A proxy is identified by resolving the import to the file that DEFINES it, so stores outside a `stores/` path (`AppLayout`, `Hardware`, the drawer stores) are covered too; a per-instance store reached through a hook handle (`pane.store.<field>`) is covered as well. Known gaps, each re-measured at 0 live conditional reads: namespace/default imports; barrel re-exports (factor 1 resolves to the barrel, not the defining file — ~109 named imports take this shape); `defineLocalStore(…).use()` locals; and an unconditional read inside a `.map()` callback (the per-iteration hazard, covered by the separate reactive-read-in-loop review angle). Fix: hoist the read above the condition, split into a component per case, or use the hook-free `.$` snapshot. A provably mount-stable condition opts out with an inline `hook-order-ok` marker (+ a reason). Detector: `scripts/lint-hooks.mjs` (`npm run lint:hooks`).
