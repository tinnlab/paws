...
> {
  ...
  render() {
    if (this.state.caught) {
      return <div data-testid="file-rag-test-boundary-crash">{this.state.caught.message}</div>
    }
...
describe('FileRagAdminPage — hook-count stability across its state transitions', () => {
  test('unauthorized -> authorized -> settings loaded -> save error (seeded-file-rag-error)', async () => {
    ...
    expect(host!.querySelector('[data-testid="file-rag-test-boundary-crash"]')).toBeNull()
  })
