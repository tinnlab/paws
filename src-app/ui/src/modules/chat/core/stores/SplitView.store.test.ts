...
test('reset() makes a subsequent auto-open a plain navigate, not a pane replace', async () => {
  ...
  const hijacked = await s().openConversationInWorkspace('c', 'auto')
  ...
  const outcome = await s().openConversationInWorkspace('d', 'auto')
  assert.equal(
    outcome.kind,
    'navigate',
