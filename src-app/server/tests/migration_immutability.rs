...
//!
//! ## The failure this exists to prevent (FB-11)
//!
//! The mirror swap was first shipped by EDITING an existing migration
//! (`202607210100_llm_repository_default_model_seed.sql`) rather than adding a
//! new one. Every install that had already applied that version then failed to
//! boot: `sqlx::migrate!` checksums each migration against `_sqlx_migrations`,
//! and the migrator is configured with `set_ignore_missing(true)` only — which
//! ignores migrations present in the DB but absent from source and does NOT
//! disable checksum validation. The mismatch aborted migrations, the embedded
//! server never started, and the UI showed "Load failed" and the first-run setup
//! page. It looks exactly like an auth or network fault, and is neither.
//!
//! A FRESH install of the same build was fine. That is precisely why it shipped:
//! every test in this repo runs against a fresh database, so the upgrade path —
//! the one real users take — was untested.
//!
//! ## WHAT THIS TEST DOES NOT COVER — read before trusting it
//!
//! This is the cheap guard, chosen deliberately over a two-stage database
//! harness. Be clear about its limits:
//!
...
//! 2. **Its baseline is what has been PUSHED**, not what has been BUILT. The
//!    build that bricked the owner's machine came from a pushed commit, so this
//!    guard would have caught it — but a build cut from a local, unpushed commit
//!    is invisible here.
//! 3. **It is skip-on-unavailable**, loudly. Without git, or in a clone with no
//!    `origin/main`, it prints why and passes rather than failing the suite.
//!
//! A real two-stage test (apply the previously-shipped set, then the current set
//! on top, against a live database) would cover (1) and (2). It needs the shared
//! test harness's database bootstrap, which rule B3 says not to reshape for one
//! feature's problem, so it is written up in FB-11 as a proposal for the owner
...
///
/// All four are on `main` and untouched by the branch that added this guard
/// (verified with `git diff origin/main...HEAD`). They are grandfathered rather
/// than "fixed": rewriting their bytes now would itself be an edit to a shipped
/// migration, which is the exact hazard this file exists to prevent.
///
