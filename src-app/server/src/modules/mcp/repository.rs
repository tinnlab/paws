...
pub(crate) struct McpServerColumnsRaw {
    ...
    // NOT NULL (migration 83); query! macros return String.
...
pub async fn update_user_mcp_server(
    ...
    let row = sqlx::query!(
        ...
        UPDATE mcp_servers SET
            ...
            sandbox_flavor = COALESCE($21, sandbox_flavor),
            updated_at = NOW()
        WHERE id = $1 AND user_id = $2 AND is_system = false
        RETURNING
            id, user_id, name, display_name, description,
...
pub async fn update_system_mcp_server(
    ...
    // The zero-config privileged built-ins (files = Track A, memory = Track B,
    ...
    let is_zero_config_builtin = existing.id
        ...
        // lit_search is config-elsewhere too (lit_search_settings + /connectors);
...
pub async fn get_system_servers_for_group(
    ...
    let rows = sqlx::query!(
        ...
        SELECT s.id, s.user_id, s.name, s.display_name, s.description,
               ...
               s.run_in_sandbox, s.sandbox_flavor,
