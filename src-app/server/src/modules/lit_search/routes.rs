...
///
/// Split from the MCP endpoint deliberately: `lit_search` is a DISABLE-only row
/// in the paws item table (design item 2), so its admin UI module stays visible
/// and unmounting these would 404 a page the design keeps. They only read and
/// write configuration — nothing here egresses a query.
...
/// on `lit_search::use`, which the Users group holds, and the runtime
/// `lit_search_settings.enabled` row defaults TRUE — so an ordinary user could
/// still drive live scholarly queries and the terms would still egress.
///
